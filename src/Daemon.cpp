...
int main() {
  ...
  Logger::init(FILE_LOGGER_DAEMON, LOGGER_DAEMON);
  ...
  const std::string queue_name = "compiler.tasks.queue";

  task.set_task_id(120);
  task.set_n_qbits(6);
  task.set_n_shots(2048);
  task.set_optimisation_level(1);
  task.set_result_destination("daemon.results.queue");
  ...
  auto res = messenger.receive<mqss::QuantumTask>(
      {"daemon.results.queue"}, mqss::ReceiveArgs{
                                    .timeout = std::chrono::milliseconds(10000),
                                    .ack_mode = mqss::AckMode::Auto,
                                });

  logger->info("Task Received by Daemon!");
  if (res.has_value()) {
