    ...
    do
    {
        ...
        do
        {
            ...
            do
            {
                ...
                flag = check_dominance(nsga2Params,
                                       &(mixed_pop->ind[temp1->index]),
        ...
        if ((archieve_size + front_size) <= nsga2Params->popsize)
        {
            do
            {
                copy_ind(nsga2Params, &mixed_pop->ind[temp2->index],
                         &new_pop->ind[i]);
        ...
        {
            crowding_fill(nsga2Params, mixed_pop, new_pop, i, front_size,
                          elite);
...
/* Routine to fill a population with individuals in the decreasing order of
 * crowding distance */
void crowding_fill(NSGA2Type *nsga2Params, population *mixed_pop,
                   population *new_pop, int count, int front_size, list *elite)
{
    int *dist;
    list *temp;
    int i, j;
    assign_crowding_distance_list(nsga2Params, mixed_pop, elite->child,
                                  front_size);
