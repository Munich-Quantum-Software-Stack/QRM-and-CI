...
void selection(NSGA2Type *nsga2Params, population *old_pop, population *new_pop)
{
    ...
    {
        parent1 = tournament(nsga2Params, &old_pop->ind[a1[i]],
                             &old_pop->ind[a1[i + 1]]);
        parent2 = tournament(nsga2Params, &old_pop->ind[a1[i + 2]],
                             &old_pop->ind[a1[i + 3]]);
        crossover(nsga2Params, parent1, parent2, &new_pop->ind[i],
                  &new_pop->ind[i + 1]);
        parent1 = tournament(nsga2Params, &old_pop->ind[a2[i]],
                             &old_pop->ind[a2[i + 1]]);
        parent2 = tournament(nsga2Params, &old_pop->ind[a2[i + 2]],
                             &old_pop->ind[a2[i + 3]]);
        crossover(nsga2Params, parent1, parent2, &new_pop->ind[i + 2],
                  &new_pop->ind[i + 3]);
    }
    free(a1);
    free(a2);
