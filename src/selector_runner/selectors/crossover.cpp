               ...
               individual *child1, individual *child2)
{
    ...
    if (nsga2Params->nint != 0)
    {
		...
		//partiallyMappedCrossover(nsga2Params, parent1, parent2, child1, child2);
		//cycleCrossover(nsga2Params, parent1, parent2, child1, child2);
	}
...
void partiallyMappedCrossover(NSGA2Type *nsga2Params, individual *parent1, individual *parent2,
         individual *child1, individual *child2) {
    ...
    if (rand <= nsga2Params->pcross_int) {
        ...
        // Copy genes from parent1 to child1 and from parent2 to child2 within the crossover sites
        for (int j = site1; j <= site2; j++) {
            child1->xint[j] = parent1->xint[j];
            child2->xint[j] = parent2->xint[j];
        }

        // Fill the remaining genes in child1 and child2 using parent2 and parent1, respectively
        for (int j = 0; j < nint; j++) {
            if (j < site1 || j > site2) {
                int gene1 = parent1->xint[j];
                int gene2 = parent2->xint[j];
                // Find the corresponding gene from parent2 in child1
                auto it = std::find(child1->xint, child1->xint + nint, gene2);
        ...
        // If crossover does not occur, copy genes from parents to children directly
        for (int j = 0; j < nint; j++) {
            child1->xint[j] = parent1->xint[j];
            child2->xint[j] = parent2->xint[j];
        }
